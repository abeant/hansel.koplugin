<p align="center">
  <img src="hansel.svg" width="420" alt="Hansel">
</p>

<p align="center"><strong>Your entire Grimmory library, straight from the home screen.</strong></p>

<p align="center">
  <a href="https://github.com/abeant/hansel.koplugin/actions/workflows/ci.yml"><img src="https://github.com/abeant/hansel.koplugin/actions/workflows/ci.yml/badge.svg" alt="CI status"></a>
  <a href="https://github.com/abeant/hansel.koplugin/releases/latest"><img src="https://img.shields.io/github/v/release/abeant/hansel.koplugin?style=flat-square&color=2f6fda&label=release" alt="Latest release"></a>
  <a href="https://koreader.rocks/"><img src="https://img.shields.io/badge/KOReader-2024.12%2B-222?style=flat-square" alt="KOReader 2024.12 or newer"></a>
  <a href="https://grimmory.org/"><img src="https://img.shields.io/badge/Grimmory-library-555?style=flat-square" alt="Grimmory"></a>
  <a href="LICENSE"><img src="https://img.shields.io/badge/license-AGPL--3.0-2f6fda?style=flat-square" alt="AGPL-3.0 license"></a>
</p>

You already have the library. You already have the reader. What's missing is a good way to get from one to the other.

Today, opening a [Grimmory](https://grimmory.org/) book on KOReader is a detour. Dig through the menu to the catalog. Page through a text list with no covers. Download something, then back out to the file browser and find where it landed. The shelves, series, and tags you organized on the server stay on the server. And once you've finished, the file sits on the device until you remember to clean it up.

Hansel makes the library the home screen. Open the device and you're looking at your covers, laid out with the shelves, genres, series, and authors you set up in Grimmory. Filter and sort in place. Tap a cover and it downloads and opens. Pin the ones that should stay put. Everything else lives on the server, one tap away. Lose the connection and the grid quietly narrows to what's already on the device, then widens again when the server is back.

<table align="center">
  <tr>
    <td align="center" valign="top"><img src="docs/screens/drawer.png" width="260" alt="Drawer listing libraries, shelves, magic shelves, genres, tags, series, and authors"></td>
    <td align="center" valign="top"><img src="docs/screens/home.png" width="260" alt="A cover grid of a Grimmory library on an e-reader"></td>
    <td align="center" valign="top"><img src="docs/screens/detail.png" width="260" alt="Book detail for The War of the Worlds, marked On server, with a Download button"></td>
  </tr>
  <tr>
    <td align="center"><sub><b>Your shelves, your order</b><br>libraries, shelves, genres, series, authors</sub></td>
    <td align="center"><sub><b>The whole library, one screen</b><br>every book you own, not just what's downloaded</sub></td>
    <td align="center"><sub><b>Tap to read</b><br>the file moves only when you say so</sub></td>
  </tr>
</table>

## Install

1. Grab [`hansel.koplugin.zip`](https://github.com/abeant/hansel.koplugin/releases/latest/download/hansel.koplugin.zip) from [Releases](https://github.com/abeant/hansel.koplugin/releases/latest). `SHA256SUMS` sits next to it if you want to check the download.
2. Unzip it into KOReader's `plugins` folder, so that `plugins/hansel.koplugin/_meta.lua` exists.
3. Restart KOReader. Enable **Hansel** under **Tools → More tools → Plugin management** if it isn't already on.
4. Open **Tools → Hansel → Show library** and enter your Grimmory URL, username, and password.

Then make it the front door: in File Manager, open the leftmost menu tab and set **Start with → Hansel**. To jump back from inside a book, bind **Show Hansel** in Gesture Manager.

<table align="center">
  <tr>
    <td align="center" valign="top"><img src="docs/screens/settings.png" width="400" alt="Hansel settings showing server URL, connection state, and auto sync"></td>
    <td align="center" valign="top"><img src="docs/screens/library-settings.png" width="400" alt="Library settings with hide unavailable books, cover prefetch, and grid density"></td>
  </tr>
  <tr>
    <td align="center"><sub><b>One server, one login</b><br>no Hansel account</sub></td>
    <td align="center"><sub><b>Yours to tune</b><br>offline behavior and grid density</sub></td>
  </tr>
</table>

## What it does

- **One grid holds everything.** Books on the server, books you've downloaded, and books you've pinned all sit in the same cover grid. No catalog to open, no downloads folder to go check.
- **Your shelves come along.** Libraries, shelves, magic shelves, genres, tags, series, and authors come straight from Grimmory, in the order you put them there.
- **Long-press a cover to decide.** Read, Download, Pin so it survives cleanup, or Remove from device. Removing touches the local copy only — the server's record is never modified.
- **Narrow it down without leaving the grid.** Filter by what's on the device, library, shelf, reading status, or format. Sort by title, author, date added, published date, series position, rating, size, or last opened.
- **Goes offline by itself.** Drop off Wi-Fi, or lose the server, and Hansel notices without being told. Covers and catalog pages are cached, so the grid still paints, and with **Hide unavailable books** on (the default) it narrows to what's genuinely on the device. When Grimmory answers again, it reconnects and widens back out on its own.
- **Progress sync is opt-in.** It's off until you turn it on, and it stands down completely if the official Grimmory plugin is already syncing — the two won't fight over your page position.
- **Built for e-ink.** Large tap targets, no animation, and three grid densities depending on how much screen you have.

<table align="center">
  <tr>
    <td align="center" valign="top"><img src="docs/screens/search.png" width="260" alt="Search dialog with Books, Authors, Tags, and Genres tabs"></td>
    <td align="center" valign="top"><img src="docs/screens/filters.png" width="260" alt="Filter sheet with on-device, library, shelf, status, and format sections"></td>
    <td align="center" valign="top"><img src="docs/screens/filters-sort.png" width="260" alt="Sort options including title, author, date added, published, and series position"></td>
  </tr>
  <tr>
    <td align="center"><sub><b>Search</b><br>books, authors, tags, genres</sub></td>
    <td align="center"><sub><b>Filter</b><br>down to what's on the device</sub></td>
    <td align="center"><sub><b>Sort</b><br>eight ways, either direction</sub></td>
  </tr>
</table>

## How it compares

Hansel isn't a replacement for either of these. They answer different questions.

| | [Grimmory plugin](https://github.com/grimmory-tools/grimmory.koplugin) | [KOReader OPDS](https://github.com/koreader/koreader/wiki/OPDS-support) | **Hansel** |
|---|---|---|---|
| The job | Sync books, shelves, and progress into KOReader | Browse any OPDS catalog | **Make the library the home screen** |
| Home screen | File Manager or KOReader's library | Catalog, then File Manager | **Your covers** |
| What you can see | Whatever you chose to sync | One feed at a time | **The entire server, on demand** |
| Keeping a book | Governed by sync rules | Download, then go find the file | **Download or pin from the book itself** |
| Offline | Your synced copies | Whatever happens to be downloaded | **Cached catalog, plus an on-device-only view** |

Reach for the official plugin when you want real two-way sync. Reach for OPDS when the catalog isn't Grimmory. Reach for Hansel when you want the library to be the first thing the device shows you.

## Pairs with Gretel

Hansel makes the library the first thing KOReader shows. [Gretel](https://github.com/abeant/gretel) makes KOReader the first thing the device shows. Put them together on an Android e-reader and the device boots straight into your Grimmory library: press Home and you're back at your covers, not at an app grid.

They're independent. Gretel is a small, free Android home app that works with any reading app. Hansel runs on every KOReader device, Android or not, with or without Gretel.

## Compatibility

| | |
|---|---|
| KOReader | 2024.12 or newer |
| Server | A reachable [Grimmory](https://grimmory.org/) instance |
| Devices | Android, Kobo, Kindle, PocketBook, reMarkable — anywhere KOReader loads plugins |
| Tested on | BOOX Nova Air, Android 10 |
| Language | Lua 5.1 / LuaJIT |
| License | [AGPL-3.0](LICENSE) |

Hansel is an independent project. It is not affiliated with or endorsed by Grimmory.

## Questions

**Do I have to keep the whole library on the reader?**
No — that's the point. Browse all of it, download what you're reading, pin what should never be cleaned up.

**What happens when Grimmory is down, or I leave the house?**
Nothing you have to manage. Hansel notices the connection is gone, paints from the last catalog it cached, and with **Hide unavailable books** on (the default) shows only your downloads and pins. When the server is reachable again it picks the rest back up automatically.

**Will it fight the official Grimmory plugin?**
No. If that plugin is handling progress sync, Hansel detects it and leaves sync alone.

**Can I still use File Manager?**
Yes. Close Hansel, or just don't set it as **Start with**.

**Does this need an account somewhere?**
Only the Grimmory login you already have. There's no Hansel service and no second account.

**Why "Hansel"?**
Breadcrumbs. Wander as deep into the collection as you like — the trail back to the library is always on screen.

## Build from source

No Grimmory server is needed to run the tests:

```sh
./tests/run.sh
luacheck --no-self _meta.lua main.lua lib ui
./scripts/package.sh
```

Details in [CONTRIBUTING.md](CONTRIBUTING.md), [docs/architecture.md](docs/architecture.md), and [CHANGELOG.md](CHANGELOG.md).

## License

[GNU Affero General Public License v3.0](LICENSE).

<p align="center"><sub>If Hansel earns a spot on your reader, a star helps other people find it.</sub></p>
